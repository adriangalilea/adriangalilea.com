  ...
  return (
    <div className="mb-3 flex items-center justify-center gap-5 text-xs text-muted-foreground">
      <span className="flex items-center gap-1.5">
        <span
          className="size-2 rounded-[2px] opacity-40"
...
/** Two lines over an ordered axis — divergence/decay stories. */
export function CompareLines({
  ...
  const deltas = new Map(
    data.map((d) => [
      d.label,
      `${d.after >= d.before ? "+" : ""}${(((d.after - d.before) / d.before) * 100).toFixed(1)}%`,
    ]),
  );
      ...
      <ChartContainer config={config} className="aspect-[2/1] w-full">
        <LineChart data={data} margin={{ top: 20, left: 24, right: 24 }}>
          ...
          <Line
            ...
            strokeOpacity={0.5}
            strokeWidth={2}
            dot={{ fill: BEFORE, strokeWidth: 0, r: 3, fillOpacity: 0.5 }}
          ...
          <Line
            ...
            stroke={AFTER}
            strokeWidth={2.5}
            dot={{ fill: AFTER, strokeWidth: 0, r: 3.5 }}
            isAnimationActive={false}
          >
            <LabelList
              position="top"
...
export function Bars({
  ...
  const rows = data.map((d) => ({
    ...d,
    fill: d.accent ? AFTER : BEFORE,
    opacity: d.accent ? 1 : 0.35,
  }));
  ...
  return (
    <figure className="not-prose my-8">
      <ChartContainer
        config={config}
        className="w-full"
        style={{ height: data.length * 58 + 8 }}
      >
        <BarChart
          data={rows}
          layout="vertical"
          margin={{ top: 18, left: 4, right: 44 }}
          barCategoryGap="38%"
        >
