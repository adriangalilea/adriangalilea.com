...
import {
  ...
  rectOf,
  sameBand,
} from "@/components/ui/lightbox-viewport";
import {
  type Action,
  type ActionId,
  ...
  resolve,
  sheet as sheetOf,
} from "@/lib/lightbox-actions";
import {
  type Flight as MotionFlight,
  planFlight,
  readFlight,
} from "@/lib/lightbox-flight";
import {
  type Gesture,
  type GestureCtx,
  type GestureEffect,
  ...
  type PointerInput,
  tapIntent,
} from "@/lib/lightbox-gesture";
import { applyHold, type HoldVerb, holdDelta } from "@/lib/lightbox-hold";
import {
  assert,
  assertSize,
  type Band,
...
import {
  ...
  wheelRelease,
  wheelTick,
} from "@/lib/lightbox-wheel";
import { phaseFeed, phaseStart } from "@/lib/lightbox-wheel-phase";
import "./lightbox.css";
...
function fitOf(m: Media, band: Band, floor?: Size): Size {
  return fit(
    boxOf(m),
    band,
    m.kind === "frame" ? FRAME_GUTTER : INSET_X,
    floor,
  );
}
...
function Stage(props: StageProps) {
  ...
  React.useLayoutEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      const next = gestureMove(
        G,
        inputOf(e, { hand: handOf(e) }),
        gestureCtx(),
      );
      G = next.gesture;
      applyGesture(next.effects);
    ...
    const fedRead = (w: WheelSession) =>
      w.phase.momentum ? " coast" : " hand";
    const endWheel = () => {
      const w = W;
      W = null;
      if (!w) return;
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      e.stopPropagation();
      if (key === null) return;
      if ((key === " " || key === "Enter") && target?.closest(ACTIVATES))
        return;
...
function Still({
  ...
  const upgrade =
    upgrades && (!!s.srcset || (s.full !== s.src && seen.current));
  return (
    <>
