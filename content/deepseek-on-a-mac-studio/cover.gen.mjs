...
const gapPoly = [
  ...after.map((v, i) => `${x(i)},${y(v)}`),
  ...before.map((v, i) => `${x(i)},${y(v)}`).reverse(),
].join(" ");

const gridLines = [400, 480, 560]
  .map(
    (v) =>
      `<line x1="${P.l - 20}" y1="${y(v)}" x2="${W - P.r + 20}" y2="${y(v)}" stroke="${GRID}" stroke-width="1"/>`,
  )
  .join("");

const dots = (arr, color, r) =>
  arr
    .map(
      (v, i) => `<circle cx="${x(i)}" cy="${y(v)}" r="${r}" fill="${color}"/>`,
    )
    .join("");
const valueLabels = (arr, color, dy, weight) =>
  arr
    .map(
      (v, i) =>
